...
class FactoryStreamMp4:
    ...
    def leer_datos(self,observer, scheduler):
        ...
        total_time = vidcap.get(cv2.CAP_PROP_FRAME_COUNT) / vidcap.get(cv2.CAP_PROP_FPS)
        frame_time = total_time / vidcap.get(cv2.CAP_PROP_FRAME_COUNT)
        delta = timedelta(seconds=frame_time)
        time_file = os.path.basename(self.archivo_mp4).split('_')[2]
        count = 0
        ...
        while (vidcap.isOpened()):
            try:
                ...
                datetime_actual = datetime.strptime(time_file, '%Y%m%d%H%M%S') + (delta * count)
