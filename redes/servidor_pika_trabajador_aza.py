...
def log_setup(path, level):
    ...
    if not os.path.isdir("logs/trabajador/"):
            os.makedirs("logs/trabajador/")
...
def inferir_imagen(ruta_imagen, model):
    ...
    for box in boxes:
        ...
        s=""+str(int(x1))+","+str(int(y1))+","+str(int(x2))+","+str(int(y2))+","+clase+","+str(confidence)+""
    
        detecciones.append(s)
    ...
    canal_envio.basic_publish(exchange='', routing_key=canal_posible_alerta, body=solo_nombre+".txt")
...
def procesar(config):    
    ...
    connection_envio = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
    canal_envio= connection_envio.channel()
    canal_envio.queue_declare(queue=canal_posible_alerta)
    canal_envio.confirm_delivery()
